...
export default function CharacterGalleryPage() {
  ...
  // Stable color index per character, independent of which group it renders under.
  const indexOf = new Map(visibleChars.map((c, i) => [c.id, i] as const));
      ...
      {hasGroups ? (
        <>
          {sections.map(section => (
            <section key={section.name} style={{ marginBottom: '28px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '0 2px 14px' }}>
                <h2 style={{ font: '700 15px var(--font-display)', letterSpacing: '.01em', margin: 0, color: section.isUngrouped ? 'var(--text-dim)' : 'var(--text-bright)' }}>
                  {section.isUngrouped ? 'Ungrouped' : section.name}
                </h2>
                <span style={{ font: '600 11px var(--font-display)', color: 'var(--text-dimmer)', background: 'var(--bg-raised)', borderRadius: '20px', padding: '2px 9px' }}>
                  {section.characters.length}
                </span>
                <div style={{ flex: 1, height: '1px', background: 'var(--border-divider)' }} />
              </div>
              <div style={gridStyle}>
                {section.characters.map(char => renderCard(char, indexOf.get(char.id) ?? 0))}
        ...
        <div style={gridStyle}>
          {visibleChars.map(char => renderCard(char, indexOf.get(char.id) ?? 0))}
          {newCharacterTile}
        </div>
      )}
